...
public class DeduceElement3_VariableTableEntry extends DefaultStateful implements IDeduceElement3 {
	...
	public GenType __getGenTypeFromInitialValue(final @NotNull IExpression iv) {
/*
		final @NotNull IExpression iv = aVariableStatement.initialValue();
		if (iv == IExpression.UNASSIGNED) {
			NotImplementedException.raise();
...
*/
		...
		case PROCEDURE_CALL -> {
			...
			case CLASS -> {
				...
				final GenType        genType1       = new GenType(classStatement);
...
//								tte.attached = vte.type.attached;
	...
	private void setup_GenType(OS_Element element, @NotNull GenType aGt) {
		...
		case NAMESPACE -> {
			...
			list.add(new SGTA_SetResolvedNamespace(namespaceStatement));
			list.add(new SGTA_RegisterNamespaceInvocation(namespaceStatement, phase));

//			pte.setNamespaceInvocation(nsi);

			list.add(new SGTA_SetNamespaceInvocation());
...
//			fi = newFunctionInvocation(fd, pte, nsi, phase);
		...
		case CLASS -> {
			...
			list.add(new SGTA_SetResolvedClass(classStatement));
			list.add(new SGTA_RegisterClassInvocation(classStatement, phase));
			list.add(new SGTA_SetClassInvocation());
