...
//			simplify_qident((Qualident) pce.getLeft(), gf); // TODO ??
	...
	private Instruction expression_to_call_add_entry(final @NotNull BaseEvaFunction gf,
		...
		final @NotNull List<TypeTableEntry>      args_types      = get_args_types(pce.getArgs(), (@NotNull EvaFunction) gf, cctx);
...
//	}
	...
	private Instruction expression_to_call_add_entry(final @NotNull BaseEvaFunction gf,
		...
		final List<TypeTableEntry>               argsTypes = get_args_types(args, (@NotNull EvaFunction) gf, cctx);
...
//			LOG.err("7056 aConstructorDef.getItem = "+item);
			generate_item(item, (BaseEvaFunction) gf, cctx);
		}
...
//		EvaFunction.printTables(gf);
	...
	public void generateFromEntryPoints(@NotNull List<EntryPoint> epl, @NotNull IClassGenerator dcg) {
		for (EntryPoint entryPoint : epl) {
			if (entryPoint instanceof final @NotNull MainClassEntryPoint mcep) {
				...
				dcg.submitGenerateClass(Objects.requireNonNull(ci), this);

				final @NotNull FunctionInvocation fi = dcg.newFunctionInvocation(f, null, ci);
//				fi.setPhase(phase);
				dcg.submitGenerateFunction(fi, this);
...
//			LOG.info("Skip namespace for now "+((NamespaceStatement) item).name());
	...
	private @NotNull InstructionArgument simplify_dot_expression(final @NotNull DotExpression dotExpression, final @NotNull BaseEvaFunction gf, Context cctx) {
	...
	private void simplify_procedure_call(@NotNull final ProcedureCallExpression pce, final @NotNull BaseEvaFunction gf, final @NotNull Context cctx) {
		...
		final int                                i = addProcTableEntry(left, expression_num, get_args_types(args, (@NotNull EvaFunction) gf, cctx), gf);
...
//			klass._a.setCode(nextClassCode());
	...
	static class GIA__procedure_call__one {
		...
		public void action(final StatementWrapper aStatementWrapper) {
			if (lookup instanceof final IntegerIA integerIA) {
		...
		int add_i(final BaseEvaFunction aGf, final InstructionName aAgn, final List<InstructionArgument> aList_of_fn_call, final Context aCctx) {
			return gfs.add_i(aGf, aAgn, aList_of_fn_call, aCctx);
		}

		int addVariableTableEntry(final String aText, final @NotNull TypeTableEntry aTte, final BaseEvaFunction aGf, final IdentExpression aIdentExpression) {
...
//				LOG.info("710 " + i);
...

				{
					for (final MatchConditional.MC1 part : mc.getParts()) {
						if (part instanceof final MatchConditionalImpl.MatchArm_TypeMatch mc1) {
							...
							gf.place(label_next);
							label_next = gf.addLabel();
						} else if (part instanceof final MatchConditionalImpl.MatchConditionalPart2 mc2) {
...
//						throw new IllegalStateException("Cant be here");
	...
	class Generate_item_assignment {
		...
		public void dot(@NotNull BaseEvaFunction gf, @NotNull DotExpression left, @NotNull IExpression right, Context cctx) {
...
//					((IdentIA) some_left).getEntry().addPotentialType(inst, unknown_type);
		...
		public void mathematical(@NotNull BaseEvaFunction gf, @NotNull IExpression left, ExpressionKind kind, @NotNull IExpression right1, Context cctx) {
...
//				idte.type = tte;
		...
		public void neg(@NotNull BaseEvaFunction gf, @NotNull IExpression left, ExpressionKind aKind, @NotNull IExpression right1, Context cctx) {
			// TODO doesn't use kind
			final @NotNull TypeTableEntry tte = gf.newTypeTableEntry(TypeTableEntry.Type.SPECIFIED, right1.getType(), right1);
...
//				idte.type = tte;
		...
		public void numeric(@NotNull BaseEvaFunction gf, @NotNull IExpression left, @NotNull NumericExpression ne, Context cctx) {
...
*/
		...
		public void string_literal(@NotNull BaseEvaFunction gf, @NotNull IExpression left, StringExpression right, Context aContext) {
...
//				LOG.info("710 " + i);
				...
				{
					for (final MatchConditional.MC1 part : mc.getParts()) {
						if (part instanceof final MatchArm_TypeMatch mc1) {
							final TypeName        tn = mc1.getTypeName();
							final IdentExpression id = mc1.getIdent();
							...
							gf.place(label_next);
							label_next = gf.addLabel();
						} else if (part instanceof final MatchConditionalPart2 mc2) {
							final IExpression id = mc2.getMatchingExpression();
