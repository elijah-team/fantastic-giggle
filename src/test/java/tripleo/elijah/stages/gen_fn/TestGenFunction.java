...
//				st.m          = om.success();
	...
	public void testBasic1Backlink1Elijah() throws Exception {
		...
		var gp = /*boilerplate.comp.getCompilationEnclosure().getPipelineLogic().*/new EvaPipeline(boilerplate.comp.get_pa());
	...
	static class TGF_Flow {
		...
		public Operation<OS_Module> addElijahFile(final String f) {
			...
			final File file = new File(f);
			try {
				return comp.use.realParseElijjahFile(f, file, false);
			} catch (Exception aE) {
